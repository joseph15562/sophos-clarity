...
function saveConfig(config: SlackConfig): void {
...
export function SlackIntegration() {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Slack test successful — message sent");
      } else {
        toast.error(`Slack returned ${res.status} — check your webhook`);
      }
    } catch {
  ...
  return (
    <div className={CARD_CLASS}>
      ...
      <p className="text-[10px] text-muted-foreground mt-1">
        Send alerts to Slack when assessments complete, scores drop, or SLA is breached.
      </p>
