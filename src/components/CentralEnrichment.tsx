...
import type { CentralFirewall, CentralAlert, CentralLicence } from "@/lib/sophos-central";
...
export function CentralEnrichment({ configMetas, customerName }: CentralEnrichmentProps) {
  ...
  useEffect(() => {
    if (!central.isConnected || !orgId || isGuest || configMetas.length === 0) return;
    ...
    const load = async () => {
      ...
      for (const tenantId of tenantIds) {
        ...
        try { alerts = await central.refreshAlerts(tenantId).then(() => central.alerts) || []; } catch { /* ignore */ }
        try {
          await central.refreshLicences(tenantId);
          licences = central.licences;
        } catch { /* ignore */ }

        for (const link of tenantLinks) {
          const config = configMetas.find((c) => c.configHash === link.config_hash);
          const fw = central.firewalls.find((f) => f.id === link.central_firewall_id) ?? null;
          results.push({
    ...
    load();
  }, [central.isConnected, orgId, configMetas.length]); // eslint-disable-line react-hooks/exhaustive-deps

  if (!central.isConnected || isGuest || enrichedData.length === 0) return null;
        ...
        <div className="border-t border-border px-4 py-3 space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {data.licences.map((l) => {
                      ...
                      return (
                        <span key={l.licenseIdentifier} className={`text-[10px] px-2 py-1 rounded border ${
          ...
          <div className="flex items-center justify-between text-[9px] text-muted-foreground pt-1">
            <span className="flex items-center gap-1">
              ...
              Data from Sophos Central · Last synced {timeAgo(central.status?.last_synced_at) || "unknown"}
            </span>
          </div>
        </div>
      )}
