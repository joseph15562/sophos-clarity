/**
 * Risk score computation — aligned with the FireComply web app.
 * Uses the same 9-category model and identical Rule Hygiene scoring.
 */
...
export function computeRiskScore(result: AnalysisResult): RiskScoreResult {
  ...
  categories.push({
    ...
    score: acPct, maxScore: 100, pct: acPct,
    details: activeWan > 0
  ...
  const mfaFindings = findings.filter((f) => f.section === "Authentication & OTP" || f.section === "OTP / MFA Settings");
  const mfaScore = mfaFindings.length === 0 ? 100 : clamp(100 - mfaFindings.length * 25);
  categories.push({
    label: "Authentication",
    score: mfaScore, maxScore: 100, pct: mfaScore,
  ...
  categories.push({
    label: "Logging",
    score: loggingPct, maxScore: 100, pct: loggingPct,
  ...
  // 6. Rule Hygiene & SSL/TLS Inspection
  ...
  categories.push({
    label: "Rule Hygiene",
    score: segScore, maxScore: 100, pct: segScore,
  ...
  const adminFindings = findings.filter((f) =>
    /admin console|ssh accessible|snmp exposed|management service.*exposed|admin services.*exposed/i.test(f.title)
  );
  ...
  categories.push({
    label: "Admin Access",
    score: adminScore, maxScore: 100, pct: adminScore,
  ...
  categories.push({
    label: "Anti-Malware",
    score: avScore, maxScore: 100, pct: avScore,
  ...
  categories.push({
    ...
    score: netScore, maxScore: 100, pct: netScore,
