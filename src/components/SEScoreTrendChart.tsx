...
export function SEScoreTrendChart({ serialNumbers, currentScore, currentGrade, seProfileId, activeTeamId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (error || !data) { setPoints([]); setLoading(false); return; }
    ...
    <div className="rounded-xl border border-border bg-card p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
          <span>{points.length} check{points.length !== 1 ? "s" : ""}</span>
      ...
      <div className="flex items-center gap-3 text-[10px]">
        <span className="text-muted-foreground">
          ...
          <Badge className={`${gradeColorClass(first.grade)} border-0 text-[9px] ml-1 px-1.5 py-0`}>{first.grade}</Badge>
        </span>
        <span className="text-muted-foreground">→</span>
        <span className="text-muted-foreground">
          Latest: <span className={`font-semibold ${deltaColor}`}>{last.score}%</span>
          <Badge className={`${gradeColorClass(last.grade)} border-0 text-[9px] ml-1 px-1.5 py-0`}>{last.grade}</Badge>
        </span>
      </div>
      ...
      <div className="w-full" style={{ minHeight: h }}>
        <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid meet" className="overflow-visible">
          {[0, 25, 50, 75, 100].map((v) => (
            <g key={v}>
              <line x1={pad.left} y1={toY(v)} x2={pad.left + chartW} y2={toY(v)} stroke="currentColor" strokeOpacity={0.08} strokeDasharray="2 2" />
              <text x={pad.left - 6} y={toY(v) + 4} textAnchor="end" className="text-[9px] fill-muted-foreground" fontSize={9}>{v}</text>
            </g>
          ))}
            ...
            <>
              ...
              {points.length > 2 && (
                <text x={pad.left + chartW / 2} y={h - 6} textAnchor="middle" className="text-[9px] fill-muted-foreground" fontSize={9}>
                  {new Date(points[Math.floor(points.length / 2)].date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                </text>
              )}
              {points.length > 1 && (
          ...
          {points.map((p, i) => {
            ...
            return (
              <g key={p.id + "-" + i}>
                ...
                {isHovered && (
                  <g>
                    <rect x={x - 50} y={y - 40} width={100} height={32} rx={6} className="fill-card stroke-border" strokeWidth={1} />
                    <text x={x} y={y - 24} textAnchor="middle" className="text-[9px] fill-muted-foreground" fontSize={9}>
                      {new Date(p.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                    </text>
                    <text x={x} y={y - 13} textAnchor="middle" className="text-[10px] font-bold fill-foreground" fontSize={10}>
              ...
              return (
                <tr key={p.id + "-row-" + i} className="border-b border-border last:border-0">
                  ...
                  <td className="px-2 py-1.5">
                    <Badge className={`${gradeColorClass(p.grade)} border-0 text-[9px] px-1.5 py-0`}>{p.grade}</Badge>
                  </td>
