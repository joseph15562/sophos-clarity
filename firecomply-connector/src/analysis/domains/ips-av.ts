...
export function analyseIpsPolicies(
    ...
    for (const row of t.rows) {
      ...
      if (action.includes("allow") || action.includes("permit")) {
        findings.push({
          ...
          remediation:
            "Go to Intrusion prevention > IPS policies. Edit the policy and set the default action to 'Drop' for maximum protection. Review IPS alerts and add exceptions only for verified false positives.",
          confidence: "high",
  ...
  if (totalPolicies === 0) {
    findings.push({
      ...
      detail:
        "No IPS policies were found in the export. Without IPS policies, intrusion prevention cannot be applied to firewall rules even if the IPS feature is licensed.",
      section: "IPS Policies",
      remediation:
        "Go to Intrusion prevention > IPS policies. Create a policy using the default template (e.g. 'lantowan_general'). Then apply it to WAN-facing firewall rules.",
      confidence: "medium",
...
export function analyseVirusScanning(
  ...
  if (disabledProtocols.length > 0) {
    findings.push({
      ...
      remediation:
        "Go to Protection > Web protection (for HTTP/HTTPS) or Email protection (for SMTP/POP3/IMAP). Enable malware scanning for each protocol. Ensure the Sophos anti-malware engine is selected and up to date.",
      confidence: "high",
  ...
  if (sandboxFound && !sandboxEnabled) {
    findings.push({
      ...
      detail:
        "The Sophos Sandstorm (sandboxing) feature is available but not enabled. Without sandboxing, zero-day malware that evades signature-based detection will not be caught. This requires a valid Sophos Central / Sandstorm licence.",
      section: "Virus Scanning",
      remediation:
        "Go to Protection > Web protection > Enable Sophos Sandstorm analysis. This sends suspicious files to the cloud sandbox for detonation analysis. Requires an active Sandstorm licence.",
      confidence: "medium",
