/**
 * Browser-safe RSS parse for Sophos Security Advisories.
 * Keep parsing logic aligned with `supabase/functions/_shared/sophos_advisory_rss.ts`.
 */
...
export interface SophosAdvisoryParsedItem {
  id: string;
  title: string;
  link: string;
  description: string;
...
function parseSophosDescription(html: string): {
  ...
  const fp = html.match(/<strong>First Published:<\/strong>[\s\S]*?datetime="([^"]+)"/i);
  return {
...
export function inferSophosAdvisoryCategory(title: string): SophosAdvisoryCategoryUi {
  const t = title.toLowerCase();

  if (/\b(wireless access|access points?|\bap6\b|wi-?fi access|wlan)\b/.test(t)) {
    return "Network";
  }
  ...
  if (/\b(sophos firewall|\bsfos\b|\butm\b|\bngfw\b)\b/.test(t) || /\bxgs\b/.test(t)) {
    return "Firewall";
  }

  if (
    /\b(endpoint|intercept x|taegis endpoint|endpoint for windows|endpoint agent|cix-)\b/.test(t)
  ) {
    return "Endpoint";
  }
...
function publishedYmd(firstPublishedIso: string | null, pubDate: string): string {
...
export function parseSophosAdvisoryRssXml(xml: string, limit: number): SophosAdvisoryParsedItem[] {
  const cap = Math.min(Math.max(limit, 1), 100);
  const blocks = xml.match(/<item[\s>][\s\S]*?<\/item>/gi) ?? [];
  const out: SophosAdvisoryParsedItem[] = [];
