...
async function generateWordBlob(markdown: string, branding: BrandingData): Promise<Blob> {
  ...
  if (branding.companyName) {
    ...
    if (branding.preparedBy) {
      headerParagraphs.push(new Paragraph({
        children: [new TextRun({ text: `Prepared by: ${branding.preparedBy}`, color: "666666", size: 20, italics: true })],
      }));
    }
    headerParagraphs.push(new Paragraph({
      children: [new TextRun({ text: new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }), color: "999999", size: 18 })],
    }));
...
async function generatePptxBlob(markdown: string, reportLabel: string, branding: BrandingData): Promise<Blob> {
  ...
  titleSlide.addText("Firewall Configuration Report", {
    x: 0.8, y: branding.customerName ? 3.8 : 3.4, w: 11, h: 0.6,
  ...
  titleSlide.addText(dateAndAuthor, {
    x: 0.8, y: branding.customerName ? 4.6 : 4.4, w: 11, h: 0.5,
