        ...
        <div className="flex items-center gap-2">
          ...
          {multipleFirewalls && (
            <div className="flex rounded-lg border border-border bg-muted/30 p-0.5">
              <button
                type="button"
                onClick={() => { setViewMode("all"); setActiveSev(null); }}
              ...
              <button
                type="button"
                onClick={() => { setViewMode("per-firewall"); setActiveSev(null); }}
                className={`px-2 py-1 text-[10px] font-medium rounded-md transition-colors ${
                  viewMode === "per-firewall" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                Per Firewall
              </button>
      ...
      {viewMode === "per-firewall" && (
        <div className="grid gap-4 sm:grid-cols-2">
          {displayPerFw.map(({ firewall, data: fwData }) => {
            ...
            return (
              <div key={firewall} className="rounded-lg border border-border bg-muted/10 p-3 space-y-2">
                <p className="text-[10px] font-semibold text-foreground uppercase tracking-wider">{firewall}</p>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {fwData.map((d) => (
                    <span key={d.key} className="text-[9px] px-1.5 py-0.5 rounded font-medium" style={{ backgroundColor: d.color + "30", color: d.color }}>
                      {d.name}: {d.value}
                    </span>
                  ))}
                </div>
