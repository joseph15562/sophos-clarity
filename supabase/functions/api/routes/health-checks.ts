...
export async function handleHealthCheckRoutes(
  ...
  if (req.method === "PATCH" && segments.length === 3 && segments[2] === "team") {
    ...
    const { data: row } = await db
      ...
      .eq("id", checkId)
      .maybeSingle();
    if (!row) return json({ error: "Health check not found" }, 404, corsHeaders);
    if (row.se_user_id !== se.seProfile.id) return json({ error: "Forbidden — you can only move your own health checks" }, 403, corsHeaders);
    ...
    if (newTeamId) {
      const { data: mem } = await db
        ...
        .maybeSingle();
      if (!mem) return json({ error: "You are not a member of that team" }, 403, corsHeaders);
    }
  ...
  if (req.method === "PATCH" && segments.length === 2 && segments[1] === "bulk-team") {
    ...
    if (ownedIds.length === 0) return json({ error: "No matching health checks found" }, 404, corsHeaders);
  ...
  if (req.method === "PATCH" && segments.length === 3 && segments[2] === "followup") {
    ...
    const { data: row } = await db
      ...
      .maybeSingle();
    if (!row) return json({ error: "Health check not found" }, 404, corsHeaders);
    if (row.se_user_id !== se.seProfile.id) return json({ error: "Forbidden" }, 403, corsHeaders);
...
<p>Schedule a follow-up session to review their progress and run a new health check.</p>`,
    ...
    return json({ ok: true, processed: (due ?? []).length, sent }, 200, corsHeaders);
  }
