  ...
  return (
    <div className="rounded-xl border border-border/70 bg-card p-5 shadow-card">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-display font-semibold tracking-tight text-foreground">Top Findings</h3>
                ...
                <div className="px-3 pb-2.5 border-t border-border/50 pt-2 space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Object.keys(analysisResults).length > 1 && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-brand-accent/10 text-brand-accent">{f.firewall}</span>
                    )}
                  </div>
                </div>
              )}
