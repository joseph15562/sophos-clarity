...
export async function handleAdminRoutes(
  ...
  if (segments[0] === "admin") {
    ...
    if (req.method === "POST" && route === "reset-mfa") {
      ...
      if (!parsed.success) {
        logJson("warn", "admin_reset_mfa_invalid_body", {
      ...
      if (factorsErr) {
        return json({ error: safeDbError(factorsErr) }, 500, corsHeaders);
      }

      const totp = factors?.factors?.filter((f: any) =>
        f.factor_type === "totp"
      ) ?? [];
  ...
  if (segments[0] === "auth" && segments[1] === "mfa-recovery") {
    ...
    if (!parsed.success) {
      logJson("warn", "auth_mfa_recovery_invalid_body", {
        issues: parsed.error.issues.length,
      });
      return json({ error: "Invalid request body" }, 400, corsHeaders);
    }
    ...
    const isAdminRecovery = callerMembership &&
      callerMembership.role === "admin" &&
      callerMembership.org_id === targetMembership.org_id;

    if (!isSelfRecovery && !isAdminRecovery) {
      return json(
        { error: "Forbidden — only org admins can reset MFA for other users" },
        403,
        corsHeaders,
      );
    }
    ...
    if (totp.length === 0) {
      return json(
        { error: "No MFA factors enrolled for this account" },
        400,
        corsHeaders,
      );
    }
      ...
      return json(
        {
          ...
          session: null,
          message:
            "MFA reset but session creation failed — sign in with your password",
        },
        200,
        corsHeaders,
      );
    ...
    if (tokenHash) {
      const { data: sessionData, error: sessionError } = await db.auth
        .verifyOtp({
    ...
    return json(
      {
        ...
        session: null,
        factorsRemoved: totp.length,
        message: "MFA factors removed — sign in with your password",
      },
      200,
      corsHeaders,
    );
