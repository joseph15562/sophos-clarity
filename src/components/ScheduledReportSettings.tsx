...
export function ScheduledReportSettings() {
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/50 bg-card/90 px-4 py-3 shadow-sm">
        <div>
      ...
      {/* Create form */}
      {showForm && (
        <div className="rounded-3xl border border-border/50 bg-gradient-to-br from-card via-card to-muted/20 p-5 space-y-5 shadow-card">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {Object.entries(formSections).map(([key, val]) => {
                ...
                return (
                  <label
                    key={key}
                    className="flex items-start gap-3 rounded-2xl border border-border/50 bg-background/80 px-3 py-3 text-sm text-foreground shadow-sm transition-colors hover:border-brand-accent/30"
                  >
                    <Checkbox
                      checked={val}
                      onCheckedChange={() =>
