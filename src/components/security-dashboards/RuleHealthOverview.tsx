...
export function RuleHealthOverview({ analysisResults }: { analysisResults: Record<string, AnalysisResult> }) {
  ...
  const cards: { key: CardKey; label: string; value: number; color: string; tooltip: string }[] = [
    { key: "totalRules", label: "Total Rules", value: stats.totalRules, color: BRAND.blue, tooltip: "Total firewall rules across all configs" },
    { key: "wanRules", label: "WAN Rules", value: stats.wanRules, color: SEVERITY_COLORS.critical, tooltip: "Rules with WAN source/destination zones — the internet-facing attack surface" },
    { key: "disabledRules", label: "Disabled", value: stats.disabledRules, color: stats.disabledRules > 0 ? SEVERITY_COLORS.high : SEVERITY_COLORS.low, tooltip: stats.disabledRules > 0 ? "Disabled rules add no security value and may indicate abandoned policy" : "No disabled rules — clean configuration" },
    ...
    { key: "hosts", label: "Hosts", value: stats.hosts, color: SEVERITY_COLORS.info, tooltip: "IP hosts/networks defined in the firewall configuration" },
    { key: "interfaces", label: "Interfaces", value: stats.interfaces, color: SEVERITY_COLORS.low, tooltip: "Physical and virtual network interfaces configured" },
  ];

  return (
    <div className="rounded-2xl border border-border/50 bg-card p-6 sm:p-7 shadow-card">
      <h3 className="text-base font-display font-bold tracking-tight text-foreground mb-5">Configuration Health</h3>
        ...
        {cards.map((c) => {
          ...
          return (
            <div key={c.key} className="relative group">
              ...
              >
                <span className="text-2xl font-display font-bold tabular-nums block" style={{ color: c.color }}>{c.value}</span>
                <span className="text-[8px] uppercase tracking-[0.12em] text-muted-foreground/70 font-semibold leading-tight mt-1 block">{c.label}</span>
              </button>
      ...
      {activeCard && isMulti && (
        <div className="mt-4 rounded-xl border border-border/40 bg-muted/5 dark:bg-muted/5 overflow-hidden">
          ...
          {perFirewall.map((fw) => {
            ...
            return (
              <div key={fw.label} className="grid grid-cols-[1fr_auto] items-center gap-3 px-4 py-2.5 border-b last:border-b-0 border-border/30 hover:bg-muted/15 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-muted/30 dark:bg-muted/20 overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: cardColor }} />
                  </div>
                </div>
                <span className="text-sm font-display font-bold tabular-nums" style={{ color: cardColor }}>{val}</span>
              </div>
            );
          })}
        </div>
