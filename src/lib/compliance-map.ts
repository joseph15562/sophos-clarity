...
const SHARED_CONTROLS: Record<string, ControlDef> = {
  dpiEngine: {
    id: "TLS-DPI",
    name: "SSL/TLS Inspection (DPI)",
    category: "Traffic Inspection",
    check: (r) => {
      const ip = r.inspectionPosture;
      if (ip.totalWanRules === 0)
        return { status: "na", evidence: "No WAN rules detected", findings: [] };
      if (ip.dpiEngineEnabled && ip.sslUncoveredZones.length === 0) {
        return {
          status: "pass",
          evidence: `${ip.sslDecryptRules} Decrypt rule${ip.sslDecryptRules !== 1 ? "s" : ""} covering all firewall WAN source zones`,
          findings: [],
        };
      }
      if (ip.dpiEngineEnabled && ip.sslUncoveredZones.length > 0) {
        ...
        return {
          status: "partial",
          evidence: `${ip.sslDecryptRules} Decrypt rule${ip.sslDecryptRules !== 1 ? "s" : ""} but ${ip.sslUncoveredZones.map((z) => z.toUpperCase()).join(", ")} zones not covered`,
          findings: f.map((x) => x.id),
        };
      }
      const f = hasFindings(r, /SSL\/TLS inspection/);
      return {
        status: "fail",
        evidence: "No SSL/TLS Decrypt rules — DPI is inactive, encrypted traffic not inspected",
        findings: f.map((x) => x.id),
      };
    },
  },
  webFilter: {
    ...
    check: (r) => {
      ...
      if (ip.webFilterableRules === 0)
        return {
          status: "na",
          evidence: "No enabled WAN rules with HTTP/HTTPS/ANY service",
          findings: [],
        };
      ...
      if (pct >= 100)
        return {
          status: "pass",
          evidence: `All ${ip.webFilterableRules} enabled WAN rules (HTTP/HTTPS/ANY) have web filtering`,
          findings: [],
        };
      if (pct >= 50)
        return {
          status: "partial",
          evidence: `${ip.withWebFilter}/${ip.webFilterableRules} enabled WAN rules (HTTP/HTTPS/ANY) filtered (${pct}%)`,
          findings: f.map((x) => x.id),
        };
      return {
        status: "fail",
        evidence: `Only ${ip.withWebFilter}/${ip.webFilterableRules} enabled WAN rules (HTTP/HTTPS/ANY) have web filtering`,
        findings: f.map((x) => x.id),
      };
    },
  },
  ips: {
    ...
    check: (r) => {
      const ip = r.inspectionPosture;
      if (ip.enabledWanRules === 0)
        return { status: "na", evidence: "No enabled WAN rules", findings: [] };
      const pct = Math.round((ip.withIps / ip.enabledWanRules) * 100);
      const f = hasFindings(r, /without IPS/);
      if (pct >= 100)
        return {
          status: "pass",
          evidence: `All ${ip.enabledWanRules} enabled WAN rules have IPS`,
          findings: [],
        };
      if (pct >= 50)
        return {
          status: "partial",
          evidence: `${ip.withIps}/${ip.enabledWanRules} enabled WAN rules have IPS`,
          findings: f.map((x) => x.id),
        };
      return {
        status: "fail",
        evidence: `Only ${ip.withIps}/${ip.enabledWanRules} enabled WAN rules have IPS`,
        findings: f.map((x) => x.id),
      };
    },
  },
  appControl: {
    ...
    check: (r) => {
      ...
      if (pct >= 80)
        return {
          status: "pass",
          evidence: `${ip.withAppControl}/${ip.enabledWanRules} enabled WAN rules have app control`,
          findings: [],
        };
      if (pct >= 40)
        return {
      ...
      return {
        status: "fail",
        evidence: `Only ${ip.withAppControl}/${ip.enabledWanRules} enabled WAN rules have app control`,
        findings: f.map((x) => x.id),
      };
    },
  },
  logging: {
    ...
    check: (r) => {
      ...
      if (f.length === 0)
        return { status: "pass", evidence: "Logging enabled on all rules", findings: [] };
  ...
  mfa: {
    ...
    check: (r) => {
      ...
      if (f.length === 0)
        return { status: "pass", evidence: "MFA/OTP enabled across all areas", findings: [] };
      return { status: "fail", evidence: f[0].detail, findings: f.map((x) => x.id) };
    },
  },
  segmentation: {
    ...
    check: (r) => {
      ...
      const all = [...broad, ...any];
      if (all.length === 0)
        return { status: "pass", evidence: "No overly broad rules detected", findings: [] };
      if (broad.length > 0 && any.length > 0)
        return {
          status: "fail",
          evidence: "Broad source/dest rules and ANY services detected",
          findings: all.map((x) => x.id),
        };
  ...
  sslInspection: {
    ...
    check: (r) => {
      ...
      if (ip.dpiEngineEnabled && ip.sslUncoveredZones.length === 0) {
        return {
          status: "pass",
          evidence: `${ip.sslDecryptRules} Decrypt rule${ip.sslDecryptRules !== 1 ? "s" : ""} with full zone coverage`,
          findings: [],
        };
      }
      if (ip.dpiEngineEnabled && ip.sslUncoveredZones.length > 0) {
        ...
        return {
          status: "partial",
          evidence: `Decrypt active but ${ip.sslUncoveredZones.map((z) => z.toUpperCase()).join(", ")} zones bypass inspection`,
          findings: f.map((x) => x.id),
        };
      }
      const f = hasFindings(r, /SSL\/TLS inspection/);
      return {
        status: "fail",
        evidence: "No SSL/TLS Decrypt rules — encrypted traffic not inspected",
        findings: f.map((x) => x.id),
      };
    },
  },
  ruleHygiene: {
    ...
    check: (r) => {
      ...
      if (dupes.length === 0)
        return { status: "pass", evidence: "No duplicate or overlapping rules", findings: [] };
  ...
  adminAccess: {
    ...
    check: (r) => {
      ...
      if (f.length === 0)
        return {
          status: "pass",
          evidence: "No management services exposed to untrusted zones",
          findings: [],
        };
      const critical = f.some((x) => x.severity === "critical");
      return {
        status: critical ? "fail" : "partial",
        evidence: f[0].detail,
        findings: f.map((x) => x.id),
      };
    },
  ...
  natSecurity: {
    id: "NAT",
    name: "NAT Rule Security",
    category: "Traffic Inspection",
    check: (r) => {
      const f = hasFindings(r, /DNAT|port forwarding|broad.*NAT/i);
      if (f.length === 0)
        return { status: "pass", evidence: "No insecure NAT rules detected", findings: [] };
      return { status: "partial", evidence: f[0].detail, findings: f.map((x) => x.id) };
    },
  },
  antiMalware: {
    id: "AV",
    name: "Anti-Malware / Virus Scanning",
    category: "Traffic Inspection",
    check: (r) => {
      const f = hasFindings(r, /virus scanning|sandboxing|zero-day/i);
      if (f.length === 0)
        return { status: "pass", evidence: "Anti-malware scanning active", findings: [] };
  ...
  vpnSecurity: {
    id: "VPN",
    name: "VPN Encryption & Authentication",
    category: "Access Control",
    check: (r) => {
      ...
      if (f.length === 0)
        return {
          status: "pass",
          evidence: "VPN profiles use strong encryption and authentication",
          findings: [],
        };
  ...
  dosProtection: {
    id: "DOS",
    name: "DoS & Spoof Protection",
    category: "Traffic Inspection",
    check: (r) => {
      const f = hasFindings(r, /dos|spoof|syn flood/i);
      if (f.length === 0)
        return { status: "pass", evidence: "DoS and spoof protection enabled", findings: [] };
      const critical = f.some((x) => x.severity === "high" || x.severity === "critical");
      return {
        status: critical ? "fail" : "partial",
        evidence: f[0].detail,
  ...
  externalLogging: {
    ...
    check: (r) => {
      ...
      if (f.length === 0)
        return {
          status: "pass",
          evidence: "External log forwarding configured (Sophos Central or syslog)",
          findings: [],
        };
      return { status: "fail", evidence: f[0].detail, findings: f.map((x) => x.id) };
    },
  ...
  wirelessSecurity: {
    ...
    check: (r) => {
      ...
      if (f.length === 0)
        return {
          status: "pass",
          evidence: "All wireless networks use strong encryption",
          findings: [],
        };
      const critical = f.some((x) => x.severity === "critical");
      return {
  ...
  snmpSecurity: {
    id: "SNMP",
    name: "SNMP Configuration Security",
    category: "Access Control",
    check: (r) => {
      ...
      if (f.length === 0)
        return {
          status: "pass",
          evidence: "SNMP uses non-default community strings",
          findings: [],
        };
      return { status: "fail", evidence: f[0].detail, findings: f.map((x) => x.id) };
    },
  ...
  cfrLogRetention: {
    id: "CFR",
    name: "Log Retention (180-day minimum)",
    category: "Monitoring & Logging",
    check: (r) => {
      const noFwd = hasFindings(r, /no external.*log.*forwarding/i);
      const hasSyslog = noFwd.length === 0;

      const isCentral =
        r.stats.sectionNames.some((s) => /central.*management/i.test(s)) ||
        !hasFindings(r, /no external.*log.*forwarding/i).some((f) =>
          /not Central-managed/i.test(f.evidence ?? ""),
        );

      if (hasSyslog && isCentral) {
        return {
          status: "pass",
          evidence:
            "External log forwarding configured (syslog + Sophos Central). Verify SIEM or CFR Xstream retention meets 180-day requirement.",
          findings: [],
        };
      }
      if (hasSyslog) {
        return {
          status: "partial",
          evidence:
            "Syslog configured but Sophos Central management not detected. Confirm SIEM retention is ≥ 180 days for PSN/CAF compliance.",
          findings: [],
        };
      }
      if (isCentral) {
        return {
          status: "partial",
          evidence:
            "Sophos Central managed (logs forwarded), but no syslog SIEM detected. Standard CFR retains 7 days; Xstream CFR retains 365 days. Verify Xstream licence or add syslog to a SIEM with ≥ 180-day retention.",
          findings: noFwd.map((x) => x.id),
        };
      }
      return {
        status: "fail",
        evidence:
          "No external log forwarding (no syslog, no Sophos Central). Firewall logs stored locally only — no path to 180-day retention. Register with Sophos Central (Xstream for 365-day CFR) or configure syslog to a SIEM.",
        findings: noFwd.map((x) => x.id),
      };
    },
  },
...
export function mapToFramework(framework: string, result: AnalysisResult): FrameworkMapping {
