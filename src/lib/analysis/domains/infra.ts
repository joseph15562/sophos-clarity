...
/** When anti-spam / email scanning sections exist, flag weakened SPF / RBL / DMARC-style toggles. */
export function analyseEmailSecuritySignals(
  sections: ExtractedSections,
  findings: Finding[],
  nextId: () => number,
) {
  const spamSection = findSection(
    sections,
    /anti-?spam|spam.*virus|smtp.*(scan|protect)|malware.*mail|email.*(gateway|protect)/i,
  );
  if (!spamSection) return;

  const blob = (
    spamSection.tables.flatMap((t) => t.rows.map((r) => JSON.stringify(r))).join(" ") +
    " " +
    (spamSection.text ?? "")
  ).toLowerCase();

  const warnings: string[] = [];
  if (/\bspf\b/.test(blob) && /\bspf\b[^.]{0,80}\b(off|disable|false|no)\b/.test(blob)) {
    warnings.push("SPF verification may be disabled");
  }
  if (
    /\brbl\b|\bdns ?bl\b|\breputation\b/.test(blob) &&
    /\b(rbl|dns ?bl|reputation)[^.]{0,100}\b(off|disable)\b/.test(blob)
  ) {
    warnings.push("RBL or reputation filtering may be disabled");
  }
  if (/\bdmarc\b/.test(blob) && /\bdmarc[^.]{0,80}\b(off|disable)\b/.test(blob)) {
    warnings.push("DMARC-related checks may be disabled where shown in export");
  }
  ...
  findings.push({
    id: `f${nextId()}`,
    severity: "medium",
    title: "Email / anti-spam configuration may weaken inbound protections",
    detail: `${warnings.join(". ")}. Compare with your organisation’s email authentication (SPF, DKIM, DMARC) and malware-scanning requirements.`,
    section: "Email Security",
    remediation:
      "Review Protect > Email and SMTP / anti-spam settings. Enable SPF verification, RBL/reputation filtering, and malware scanning for inbound mail where appropriate.",
    confidence: "low",
    evidence: `Email/anti-spam section present; heuristics: ${warnings.join("; ")}`,
  });
}
