...
export function analyseHA(sections: ExtractedSections, findings: Finding[], nextId: () => number) {
  const section =
    findSection(sections, /^HAConfigure$/i) ?? findSection(sections, /high.?availability/i);
  if (!section) {
    findings.push({
      id: `f${nextId()}`,
      severity: "info",
      ...
      detail:
        "No HA configuration section was found. This firewall appears to be running as a standalone device without active-passive or active-active failover.",
      section: "High Availability",
      remediation:
        "Consider deploying a secondary Sophos firewall in HA mode (active-passive or active-active) for hardware redundancy and business continuity.",
      confidence: "medium",
  ...
  for (const d of section.details ?? []) {
    ...
    for (const [k, v] of Object.entries(f)) {
      ...
      if (
        !deviceMode &&
        (kl === "device" || kl === "hamode" || kl.endsWith(".device") || kl.endsWith(".hamode"))
      ) {
  ...
  if (!deviceMode) {
    ...
    deviceMode = text.match(
      /(?:Device|HAMode)[=":,\s]*(Active[_\s-]?Passive|Active[_\s-]?Active|Standalone)/i,
    )?.[1];
