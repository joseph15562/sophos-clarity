...
async function handleShared(
  ...
  const { data, error } = await db
    .from("shared_reports")
    .select(
      "share_token, markdown, customer_name, expires_at, created_at, allow_download, advisor_notes",
    )
  ...
  return json(
    {
      ...
      allow_download:
        (data as { allow_download?: boolean }).allow_download !== false,
      advisor_notes:
        (data as { advisor_notes?: string | null }).advisor_notes ?? null,
    },
    200,
    corsHeaders,
  );
...
async function handleSharedHealthCheck(
  token: string,
  corsHeaders: Record<string, string>,
) {
  const db = adminClient();
  const { data, error } = await db
    ...
    .select(
      "share_token, shared_html, customer_name, share_expires_at, checked_at",
    )
    .eq("share_token", token)
    .maybeSingle();
  ...
  if (expiresAt <= new Date()) {
    return json(
      { error: "This shared health check has expired" },
      410,
      corsHeaders,
    );
  }
  ...
  return json(
    {
      ...
      session: null,
      message:
        "Passkey verified but session creation failed — please sign in with password",
    },
    200,
    corsHeaders,
  );
...
async function handleConfigUploadPublic(
  ...
  if (req.method === "GET" && !subRoute) {
    ...
    const { data, error } = await db
      ...
      .select(
        "status, customer_name, expires_at, file_name, central_connected_at, central_data, central_linked_firewall_id, central_linked_firewall_name",
      )
      .eq("token", token)
      .maybeSingle();
    ...
    if (error) return json({ error: safeDbError(error) }, 500, corsHeaders);
    if (!data) {
      return json({ error: "Upload link not found" }, 404, corsHeaders);
    }
    if (new Date(data.expires_at) <= new Date()) {
      return json({ error: "This upload link has expired" }, 410, corsHeaders);
    }
    ...
    return json(
      {
        ...
        central_tenants: data.central_data
          ? (data.central_data as Record<string, unknown>).tenants ?? null
          : null,
        central_firewalls: data.central_data
          ? (data.central_data as Record<string, unknown>).firewalls ?? null
          : null,
        central_account_type: data.central_data
          ? (data.central_data as Record<string, unknown>).accountType ?? null
          : null,
  ...
  if (req.method === "POST" && !subRoute) {
    ...
    if (fetchErr) {
      return json({ error: safeDbError(fetchErr) }, 500, corsHeaders);
    }
    if (!existing) {
      return json({ error: "Upload link not found" }, 404, corsHeaders);
    }
    ...
    if (existing.status === "downloaded") {
      return json(
        {
          error:
            "This configuration has already been downloaded by the SE. Please request a new upload link.",
        },
        409,
        corsHeaders,
      );
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (file.size > MAX_CONFIG_SIZE) {
        return json({ error: "File exceeds 10 MB limit" }, 413, corsHeaders);
      }
    ...
    if (!xmlContent.trim()) {
      return json({ error: "Empty file" }, 400, corsHeaders);
    }
    ...
    if (!isValidSophosXml(xmlContent)) {
      return json(
        {
          error:
            "This doesn't appear to be a Sophos firewall configuration export. Please export your entities.xml from Sophos Firewall and try again.",
        },
        422,
        corsHeaders,
      );
    ...
    if (updateErr) {
      return json({ error: safeDbError(updateErr) }, 500, corsHeaders);
    }
    ...
    if (existing.se_email) {
      ...
      if (freshRow?.central_connected_at) {
        centralNote = freshRow.central_linked_firewall_name
          ? `<p style="margin:0 0 12px;">The customer also connected <strong>Sophos Central</strong> (linked to firewall: <strong>${
            escapeHtml(freshRow.central_linked_firewall_name)
          }</strong>).</p>`
      ...
      if (existing.se_user_id) {
        const { data: seProfile } = await db
          ...
          .maybeSingle();
        seName = seProfile?.display_name ||
          seProfile?.health_check_prepared_by || "";
      }
      const notifHtml = buildSeNotificationEmailHtml(
      ...
      await sendConfigUploadEmail(
        existing.se_email,
        `Config received${
          existing.customer_name ? ` from ${existing.customer_name}` : ""
        }`,
        notifHtml,
      );
    }
    ...
    return json(
      { ok: true, message: "Configuration uploaded successfully" },
      200,
      corsHeaders,
    );
  }
  ...
  if (req.method === "POST" && subRoute === "central-connect") {
    ...
    if (!row) {
      return json({ error: "Upload request not found" }, 404, corsHeaders);
    }
    ...
    try {
      const accessToken = await sophosGetToken(
        client_id.trim(),
        client_secret.trim(),
      );
      ...
      return json(
        { error: safeError(err, "Central connection failed") },
        400,
        corsHeaders,
      );
    }
  ...
  if (req.method === "POST" && subRoute === "central-firewalls") {
    const { data: row, error: fetchErr } = await db
      .from("config_upload_requests")
      .select(
        "id, expires_at, central_client_id_enc, central_client_secret_enc, central_data",
      )
      .eq("token", token)
      .maybeSingle();
    if (fetchErr) {
    ...
    if (!row.central_client_id_enc) {
      return json({ error: "Central not connected yet" }, 400, corsHeaders);
    }

    const body = await req.json();
    const { tenant_id } = body as { tenant_id?: string };
    if (!tenant_id?.trim()) {
      return json({ error: "tenant_id is required" }, 400, corsHeaders);
    }

    try {
      const clientId = await centralDecrypt(
        row.central_client_id_enc,
        async (re) => {
          await db.from("config_upload_requests").update({
            central_client_id_enc: re,
          }).eq("id", row.id);
        },
      );
      const clientSecret = await centralDecrypt(
        row.central_client_secret_enc,
        async (re) => {
          await db.from("config_upload_requests").update({
            central_client_secret_enc: re,
          }).eq("id", row.id);
        },
      );
      ...
      const updatedCentralData = {
        ...(row.central_data as Record<string, unknown>),
        ...fwData,
        selectedTenantId: tenant_id.trim(),
      };
      await db
        .from("config_upload_requests")
      ...
      return json(
        { error: safeError(err, "Failed to fetch firewalls") },
        400,
        corsHeaders,
      );
    }
  ...
  if (req.method === "POST" && subRoute === "central-link") {
    ...
    if (!row.central_connected_at) {
      return json({ error: "Central not connected yet" }, 400, corsHeaders);
    }

    const body = await req.json();
    const { firewall_id, firewall_name } = body as {
      firewall_id?: string;
      firewall_name?: string;
    };
    if (!firewall_id?.trim()) {
      return json({ error: "firewall_id is required" }, 400, corsHeaders);
    }
...
export async function handleApiPublicRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    // GET /shared/:token
    if (
      req.method === "GET" && segments[0] === "shared" && segments.length === 2
    ) {
    ...
    if (
      req.method === "GET" && segments[0] === "shared-health-check" &&
      segments.length === 2
    ) {
    ...
    if (
      req.method === "POST" && segments[0] === "passkey" &&
      segments[1] === "login-options"
    ) {
    ...
    if (segments[0] === "config-upload" && segments.length >= 2) {
      ...
      const result = await handleConfigUploadPublic(
        req,
        token,
        subRoute,
        corsHeaders,
