...
function IssueCard({ issue }: { issue: RuleIssue }) {
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-card/90 overflow-hidden shadow-card">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-muted/20 transition-colors">
        <div className={`h-8 w-8 rounded-xl ${cfg.bg} flex items-center justify-center shrink-0 border border-transparent`}>
      ...
      {open && (
        <div className="px-4 pb-4 pt-3 border-t border-border/70 bg-muted/[0.08] space-y-2.5">
  ...
  return (
    <div className="rounded-[28px] border border-brand-accent/15 bg-[radial-gradient(circle_at_top_left,rgba(32,6,247,0.10),transparent_34%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(247,249,255,0.98))] dark:bg-[radial-gradient(circle_at_top_left,rgba(32,6,247,0.18),transparent_34%),linear-gradient(135deg,rgba(9,13,24,0.98),rgba(12,18,34,0.98))] p-5 sm:p-6 shadow-[0_18px_50px_rgba(32,6,247,0.08)] space-y-4" data-tour="rule-optimiser">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="space-y-2 min-w-[220px]">
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-display font-black text-foreground tracking-tight">Rule Optimisation</h3>
          </div>
          <p className="text-[11px] text-muted-foreground leading-relaxed max-w-2xl">
            Surface duplicate, shadowed, and mergeable policy entries so MSP engineers and SE teams can quickly explain rule hygiene improvements.
          </p>
        </div>
        {totalIssues > 0 ? (
          <div className="rounded-2xl border border-[#F29400]/15 bg-[#F29400]/10 px-4 py-3 text-right">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#c47800] dark:text-[#F29400]">Issues detected</p>
            <p className="text-3xl font-black text-[#c47800] dark:text-[#F29400] mt-1">{totalIssues}</p>
          </div>
        ) : (
          <div className="rounded-2xl border border-[#00F2B3]/15 bg-[#00F2B3]/10 px-4 py-3 text-right">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#00774a] dark:text-[#00F2B3]">Policy state</p>
      ...
      {Object.entries(results).map(([label, result]) => (
        <div key={label} className="space-y-3 rounded-2xl border border-border/70 bg-card/70 p-4 shadow-card">
