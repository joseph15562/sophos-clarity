...
- **Order for ≤150 rules**: If the payload has **150 or fewer** firewall rules, output **all** rule rows in the **## Firewall Rules** table **before** writing **Summary of Firewall Rules**, **Findings**, or **Best Practice** for that section — do not stop mid-table to write findings or narrative.
...
- **Firewall rules table columns**: Use **exactly** these columns in this order: **#**, **Rule Name**, **Status**, **Policy Type**, **Action**, **Source Zone**, **Source Networks**, **Destination Zone**, **Destination Networks**, **Service**, **Web Filter**, **IPS Policy**, **Application Control**, **Match Known Users**, **Identity**, **Log**. Do **not** add any other columns (no Description, no Schedule, no Security Features, no Heartbeat columns, no duplicate zone/IPS/log columns). Merge Web Filter and Logging from detail blocks into these columns. If the payload has a "#" column, use it as the first column; otherwise use row order (1, 2, 3, …). **Use exactly "Enabled" or "Disabled"** (capitalised) for Status, Log, and any on/off fields. If no web filter data for a rule, show "None"; **only call it out in Findings when the rule's Destination Zone is WAN and Service is HTTP, HTTPS, or ANY** — rules with destination other than WAN or non-web services do not require web filtering for compliance.
...
**2. Control to Evidence Mapping Tables**
