...
| `alerts`            | `GET {apiHost}/common/v1/alerts`                                    | `X-Tenant-ID`                         | Paginated; Postman uses `pageFromKey` + `pages.nextKey` (cursor) as well as `page`/`pageSize`/`pageTotal`. Edge follows `nextKey` when present, else numeric pages. No GET `sort=` (some tenants 400). Newest-first after merge in Edge |
