...
function SetupStep({
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      ...
      >
        <span className="flex items-center justify-center h-6 w-6 rounded-full bg-brand-accent/10 text-brand-accent text-xs font-bold shrink-0">
          {step}
        </span>
...
export function CentralIntegration() {
  ...
  const handleDisconnect = useCallback(async () => {
    if (
      !confirm(
        "Disconnect from Sophos Central? This will remove stored credentials and cached data.",
      )
    )
      return;
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      {central.isConnected ? (
        <div className="rounded-lg border border-[#008F69]/30 dark:border-[#00F2B3]/20 dark:border-[#008F69]/30 dark:border-[#00F2B3]/20 bg-[#00F2B3]/[0.04] dark:bg-[#00F2B3]/[0.04] px-4 py-3 flex items-center gap-3">
          <div className="h-8 w-8 rounded-lg bg-[#008F69]/[0.12] dark:bg-[#00F2B3]/10 dark:bg-[#008F69]/[0.12] dark:bg-[#00F2B3]/10 flex items-center justify-center shrink-0">
            <Wifi className="h-4 w-4 text-[#007A5A] dark:text-[#00F2B3]" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-foreground">Sophos Central Connected</p>
            <p className="text-[10px] text-muted-foreground">
              Type:{" "}
              <span className="font-medium text-foreground">{central.status?.partner_type}</span>
              {" · "}Last synced:{" "}
              <span className="font-medium text-foreground">
                {timeAgo(central.status?.last_synced_at)}
              </span>
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {central.status?.partner_type !== "tenant" && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => central.refreshTenants()}
                disabled={central.isLoading}
                className="gap-1.5 text-xs"
              >
                <RefreshCw className={`h-3 w-3 ${central.isLoading ? "animate-spin" : ""}`} />
                Sync Tenants
                {central.tenants.length > 0 && (
                  <span className="text-[10px] opacity-60">({central.tenants.length})</span>
                )}
              </Button>
            )}
            <Button
              ...
              className="gap-1.5 text-xs text-[#EA0022] hover:text-[#EA0022] hover:bg-[#EA0022]/10"
            >
        ...
        <div className="space-y-4" data-tour="central-section">
            ...
            <div className="space-y-2">
              <SetupStep step={1} title="Sign in to Sophos Central" defaultOpen>
                <p>
                  ...
                  >
                    central.sophos.com <ExternalLink className="h-2.5 w-2.5" />
                  </a>
                </p>
                <p className="mt-1">
                  Partners: use the Partner dashboard. Customers: use Sophos Central Admin.
                </p>
              </SetupStep>
              <SetupStep step={2} title="Navigate to API Credentials">
                <p>
                  Go to <span className="font-medium text-foreground">Settings & Policies</span> →{" "}
                  <span className="font-medium text-foreground">API Credentials Management</span>
                </p>
              </SetupStep>
              <SetupStep step={3} title='Add a new credential named "FireComply"'>
                <p>
                  Click <span className="font-medium text-foreground">Add Credential</span>, enter
                  the name <span className="font-mono bg-muted px-1 rounded">FireComply</span>, and
                  select the role:
                </p>
                <div className="mt-2 rounded border border-border overflow-hidden">
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-[#008F69]/[0.08] dark:bg-[#00F2B3]/5">
                    <Shield className="h-3 w-3 text-[#00F2B3]" />
                    <span className="font-medium text-foreground">Service Principal Read-Only</span>
                    <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded bg-[#008F69]/[0.12] dark:bg-[#00F2B3]/10 text-[#00F2B3] font-bold">
                      RECOMMENDED
                    </span>
                  </div>
                  <p className="px-3 py-1.5 text-[10px]">
                    View-only access. FireComply never modifies your Sophos Central configuration.
                  </p>
                </div>
              </SetupStep>
              <SetupStep step={4} title="Copy Client ID and Client Secret">
                <p>
                  After clicking Add, copy the{" "}
          ...
          <div className="space-y-3 pt-1">
            ...
            {connectError && (
              <p className="text-xs text-[#EA0022] bg-[#EA0022]/5 rounded px-3 py-2">
                {connectError}
              </p>
            )}
