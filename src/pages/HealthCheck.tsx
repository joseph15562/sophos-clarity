...
function HealthCheckInner() {
  ...
  const handleSendReportToCustomer = useCallback(async () => {
    ...
    try {
      const { buildSeHealthCheckPdfBlob } = await import("@/lib/se-health-check-pdfmake");
      const { buildSeHealthCheckBrowserHtmlDocument } = await import("@/lib/se-health-check-browser-html");
