  ...
  if (threatStatus.atp && fwMajor >= 19) {
    if (!threatStatus.atp.enabled) {
      findings.push({
        ...
        detail:
          "Active Threat Response provides real-time threat intelligence from Sophos X-Ops. Disabling it removes automatic blocking of known malicious indicators.",
      ...
      findings.push({
        ...
        detail:
          "Active Threat Response is enabled but set to 'Log Only'. Detected threats are logged but not blocked. Consider switching to 'Log and Drop' for active protection.",
        section: "Threat Protection",
        confidence: "high",
      });
    }
  ...
  if (fwMajor >= 21.5) {
    if (threatStatus.ndr && !threatStatus.ndr.enabled) {
      findings.push({
        ...
        detail:
          "Network Detection and Response Essentials provides deep network traffic analysis for advanced threat detection. Enable NDR to gain visibility into lateral movement and encrypted traffic anomalies.",
        section: "Threat Protection",
        confidence: "high",
      });
    }
