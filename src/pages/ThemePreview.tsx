import { Shield, AlertTriangle, FileCheck2, TrendingUp, Zap, Clock, Clock3, Sparkles, Upload, Play, Lock, Cpu, ChevronRight, BarChart3, Globe, FileSearch, ArrowRight, CheckCircle2, Moon, Gauge } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#00102e] text-white">
      ...
      <main id="main-content" className="max-w-6xl mx-auto px-6 py-8 space-y-10">
        ...
        <section className="rounded-[28px] border border-brand-accent/15 bg-[radial-gradient(circle_at_top_left,rgba(32,6,247,0.18),transparent_40%),radial-gradient(circle_at_bottom_right,rgba(0,237,255,0.10),transparent_35%),linear-gradient(135deg,rgba(0,16,46,0.98),rgba(0,24,64,0.98))] shadow-[0_20px_60px_rgba(32,6,247,0.08)] relative overflow-hidden">
          ...
          <div className="text-center pt-10 pb-6 px-6 space-y-5">
            <h2 className="text-3xl font-display font-black text-white tracking-tight">
              Turn Sophos Firewall Exports into<br />
              <span className="bg-gradient-to-r from-[#2006F7] via-[#5A00FF] to-[#00EDFF] bg-clip-text text-transparent">Audit-Ready Documentation</span>
            </h2>
            <p className="text-sm text-white/50 max-w-xl mx-auto leading-relaxed">
              ...
              and compliance mapping — no AI required.
            </p>
          ...
          <div className="border-t border-white/[0.06] px-5 py-4 space-y-3">
            <div className="grid gap-2 sm:grid-cols-3">
              ...
              <div className="rounded-xl border border-[#00F2B3]/20 bg-[#00F2B3]/[0.05] px-3 py-2 text-left">
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/35">With FireComply</p>
              ...
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-left">
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/35">Effort saved</p>
        ...
        <div className="relative overflow-hidden rounded-[28px] border border-brand-accent/15 bg-[radial-gradient(circle_at_top_left,rgba(32,6,247,0.22),transparent_35%),radial-gradient(circle_at_top_right,rgba(0,242,179,0.12),transparent_28%),linear-gradient(135deg,rgba(0,16,46,0.98),rgba(0,24,64,0.98))] shadow-[0_20px_60px_rgba(32,6,247,0.08)] p-6 sm:p-7 space-y-6">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="grid grid-cols-3 gap-2.5 min-w-[340px]">
              ...
              <MiniCard label="With FireComply" value="< 2 minutes" sub="Demo-ready outcome" accent="cyan" />
              <MiniCard label="Effort saved" value="90%+" sub="Assessment & reporting" accent="green" />
            </div>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <Pill icon={<AlertTriangle className="h-3.5 w-3.5" />} label="Critical / High" value="8" color="red" />
            <Pill icon={<Shield className="h-3.5 w-3.5" />} label="Security Coverage" value="25%" color="red" />
            <Pill icon={<FileCheck2 className="h-3.5 w-3.5" />} label="Compliance" value="Mapped" color="green" />
            <Pill icon={<TrendingUp className="h-3.5 w-3.5" />} label="Report Readiness" value="Generate" color="neutral" />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#001840] p-4 space-y-4">
          <h2 className="text-sm font-display font-bold text-white tracking-tight px-1 mb-2">Detailed Security Analysis</h2>
          <div className="inline-flex h-9 items-center justify-center rounded-lg bg-white/[0.04] p-1 text-white/40">
            {["Overview", "Security Analysis", "Compliance", "Optimisation", "Tools", "Remediation"].map((t, i) => (
              <button
                key={t}
              ...
              >
                {t}
                {i === 0 && <span className="ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#EA0022]/10 text-[#EA0022] tabular-nums">10</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#001840] p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/40">Ranked by severity and expected score improvement</p>
            </div>
          </div>
          {[
            { sev: "critical", title: "3 enabled WAN rules missing web filtering", impact: "+12 pts", section: "Firewall Rules", evidence: "Guest-WiFi, Allow-All-Temp have Web Filter=none" },
            { sev: "critical", title: "4 fully open rules (any source, destination, service)", impact: "+10 pts", section: "Firewall Rules", evidence: "Allow-All-Temp, Old-VPN-Policy have Source=Any" },
            { sev: "high", title: "3 rules with logging disabled", impact: "+6 pts", section: "Firewall Rules", evidence: "Guest-WiFi, Old-VPN-Policy have Log=disabled" },
            { sev: "high", title: "No IPS policy on WAN-facing rules", impact: "+5 pts", section: "IPS Configuration", evidence: "0/7 enabled WAN rules have IPS" },
            { sev: "high", title: "SSL/TLS inspection not configured", impact: "+4 pts", section: "SSL/TLS Rules", evidence: "No SSL/TLS inspection rules found" },
          ].map((a, i) => (
            <div key={i} className={`rounded-xl border border-white/[0.06] border-l-[3px] ${a.sev === "critical" ? "border-l-[#EA0022]" : "border-l-[#F29400]"} bg-[#001d4d] overflow-hidden`}>
              <div className="flex items-start gap-3 px-4 py-3">
                <span className="shrink-0 text-xs font-black text-white/30 tabular-nums w-5 text-right mt-0.5">{i + 1}</span>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${a.sev === "critical" ? "bg-[#EA0022]/15 text-[#EA0022]" : "bg-[#F29400]/15 text-[#F29400]"}`}>{a.sev}</span>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#001840] p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              <label key={t.label} className={`flex items-center gap-2.5 rounded-xl border px-3 py-2.5 cursor-pointer transition-colors ${t.checked ? "border-[#00EDFF]/30 bg-[#00EDFF]/[0.05]" : "border-white/[0.06] bg-[#001d4d] hover:bg-white/[0.02]"}`}>
                <div className={`h-4 w-4 rounded border-2 flex items-center justify-center ${t.checked ? "border-[#00EDFF] bg-[#00EDFF]" : "border-white/20"}`}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 rounded-xl border border-[#00F2B3]/15 bg-[#00F2B3]/[0.03] p-4">
            <div className="text-center">
              ...
              <p className="text-2xl font-black text-white">58 <ArrowRight className="inline h-4 w-4 text-[#00F2B3]" /> <span className="text-[#00F2B3]">82</span></p>
            </div>
            <div className="text-center">
              <p className="text-[10px] font-semibold text-white/40 uppercase tracking-wider">Grade</p>
              <p className="text-2xl font-black text-white">D <ArrowRight className="inline h-4 w-4 text-[#00F2B3]" /> <span className="text-[#00F2B3]">B</span></p>
            </div>
            <div className="text-center">
              <p className="text-[10px] font-semibold text-white/40 uppercase tracking-wider">Findings Resolved</p>
              <p className="text-2xl font-black text-[#00F2B3]">8</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] font-semibold text-white/40 uppercase tracking-wider">Coverage</p>
              <p className="text-2xl font-black text-white">25% <ArrowRight className="inline h-4 w-4 text-[#00F2B3]" /> <span className="text-[#00F2B3]">100%</span></p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#001840] p-5 space-y-3">
          <div className="flex items-start gap-3 rounded-xl border border-white/[0.06] bg-[#001d4d] px-4 py-3">
            <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded bg-[#EA0022]/15 text-[#EA0022] mt-0.5">critical</span>
            <div className="flex-1 space-y-2.5">
              <p className="text-xs font-semibold text-white">3 enabled WAN rules missing web filtering</p>
              <p className="text-[11px] text-white/50 leading-relaxed">
                Web filtering is not applied to 3 enabled WAN rules with HTTP/HTTPS/ANY service. This allows users to access malicious or policy-violating websites without protection.
              </p>
              <div className="rounded-lg bg-white/[0.03] border border-white/[0.06] px-3 py-2 space-y-1">
                <p className="text-[9px] font-bold uppercase tracking-wider text-white/30">Evidence Source</p>
                <p className="text-[10px] text-white/60">
                  <span className="font-medium text-white/80">Section:</span> Firewall Rules
                  <br />
                  <span className="font-medium text-white/80">Extracted fact:</span> <span className="font-mono">Rules Guest-WiFi-Internet, Allow-All-Temp, Legacy-Allow-All have Web Filter=none/empty</span>
                </p>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#001840] p-5">
          ...
          <div className="rounded-xl overflow-hidden border border-white/[0.06]" style={{ aspectRatio: "2.5/1", background: "#000d24" }}>
            <div className="w-full h-full flex items-center justify-center text-white/20 text-xs">
              [ World Map Renders Here ]
            </div>
          </div>
          <div className="flex gap-4 mt-2 text-[10px] text-white/40">
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#F29400]" /> Exposed</span>
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#EA0022]" /> Has CVEs</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {["Export Risk Register (CSV)", "Export Excel", "Export Interactive HTML", "Generate All Reports"].map((label, i) => (
            <button
              key={label}
          ...
          {[
            ...
            <div key={c.label} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-2.5 rounded-full bg-white/[0.06] overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${c.pct}%`, backgroundColor: c.color, boxShadow: `0 0 8px ${c.color}40` }} />
              </div>
              <span className="text-[11px] font-bold tabular-nums w-10 text-right" style={{ color: c.color }}>{c.pct}%</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-12 gap-2">
          {Object.entries(BRAND).map(([name, hex]) => (
            <div key={name} className="flex flex-col items-center gap-1.5">
              <div className="h-10 w-10 rounded-lg border border-white/10" style={{ backgroundColor: hex }} />
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 pt-4">
      ...
      <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#00EDFF]/60 whitespace-nowrap">{children}</span>
      <div className="h-px flex-1 bg-gradient-to-l from-[#2006F7]/40 to-transparent" />
    </div>
  );
}

function MiniCard({ label, value, sub, accent }: { label: string; value: string; sub: string; accent?: "cyan" | "green" }) {
  const border = accent === "cyan" ? "border-[#00EDFF]/20" : accent === "green" ? "border-[#00F2B3]/20" : "border-white/[0.06]";
  const bg = accent === "cyan" ? "bg-[#00EDFF]/[0.05]" : accent === "green" ? "bg-[#00F2B3]/[0.05]" : "bg-white/[0.02]";
  const valColor = accent === "cyan" ? "text-[#00EDFF]" : accent === "green" ? "text-[#00F2B3]" : "text-white";
  return (
...
function Pill({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: string; color: "red" | "amber" | "green" | "neutral" }) {
  const styles = {
...
function StatCard({ label, value, badge, color }: { label: string; value: string; badge?: string; color: "red" | "green" | "neutral" }) {
  const styles = {
    red: "border-[#EA0022]/20 bg-[#EA0022]/[0.05]",
    green: "border-[#00F2B3]/20 bg-[#00F2B3]/[0.05]",
    neutral: "border-white/[0.06] bg-white/[0.02]",
  };
  const valColor = color === "red" ? "text-[#EA0022]" : color === "green" ? "text-[#00F2B3]" : "text-white";
  return (
    <div className={`rounded-2xl border ${styles[color]} p-4`}>
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        {badge && <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${color === "red" ? "bg-[#EA0022]/15 text-[#EA0022]" : "bg-[#00F2B3]/15 text-[#00F2B3]"}`}>{badge}</span>}
      </div>
    </div>
  );
}
